...
GLuint vboIDs[18]; // Two Vertex Buffer Objects for each VAO (specifying vertex positions and colours, respectively)
...
void initBoard()
{
	...
	for (int i = 0; i < 1200; i++) boardcolours[i] = black; // Let the empty cells on the board be black
...
bool isLegalPos(vec2 cur_tilepos){
	...
	//check that they are INSIDE the board coords 1 <= x <= 10, 1 <= y <= 20;
	//ALSO check that they are not over an occupied tile - check ALL FOUR
	//(x,y) positions in all_cell_pos against board[x][y] --> if board[x][y]
